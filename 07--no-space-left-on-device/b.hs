...
solve text =
  ...
  & (\(fileSizes, dirs) ->
      let
        ...
        desiredFreeSpace = 30_000_000
        currentUsedSpace = totalSize fileSizes []
        currentFreeSpace = totalDiskSpace - currentUsedSpace
        needed = desiredFreeSpace - currentFreeSpace
      in
        dirs
